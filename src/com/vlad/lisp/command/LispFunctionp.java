...
public class LispFunctionp extends LispAbstractCommand {

	public LispFunctionp(LispList parameters, LispGlobal global) throws LispException {
	...
	public LispAtom execute(LispArguments arguments) throws LispException {
		...
		LispAtom atom = getParameters().get(1);

		if (atom.isAtom() && !atom.isNil()) {
			result = new LispBoolean(LispCommandFactory.isValidCommandName((String) atom.getValue())
					|| getGlobal().isDefinedFunction((String) atom.getValue()));
		} else {
